...
function command_scan(client_obj, delimiter_open, delimiter_close) {
    ...
    let reg_ex = new RegExp(delimiter_open + '[a-zA-Z0-9]+' + delimiter_close);
    ...
    client_obj.on('messageCreate', async (message, interaction) => {
        ...
        if(command_list == null){
            ...
            author.send("The command you have sent to the bot is: " + command_list.toString());
            
            // !help command lists some of the available bot commands.
            if( command === '!help' ){
                const helpEmbed = new MessageEmbed()
                .setColor('#0099ff') 
                .setTitle('Commands')
                //.setAuthor(author.username + "'s User Profile")
                .setDescription('!help: Displays this list.\n!test: Reacts with 👌.\n!profile: User profile with drop down menu.')
                .setTimestamp();
                
                message.channel.send({embeds: [helpEmbed]})
            }
            
            //Code to react to the user who used the !test command
            if( command === '!test'){
                message.react('👌');
            }
            ...
            if( command === '!profile'){
                ...
                await author.send({embeds: [tagEmbed], components: [Row1]})
